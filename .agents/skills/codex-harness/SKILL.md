...
if manifest_version != "0.1.5" or "version: 0.1.5" not in skill_text:
...
- `tasks/<task-dir>/context-pack/runtime/phase<N>-attempt-manifest.jsonl` for every executed phase
...
`phase<N>-attempt-manifest.jsonl` is the attempt lifecycle ledger. `phase<N>-result-attempt<M>.json` and `phase<N>-attempt<M>-commit.json` are the canonical completion proof. `phase<N>-result.json` is a latest alias for humans and compatibility. Phase agents write handoffs, not result JSON.
