...
Codex로 며칠짜리 작업을 하다 보면 대화가 금방 무거워집니다.

처음에 정한 기준은 묻히고, 중간에 버린 아이디어나 틀린 가정이 다음 작업에 슬쩍 섞입니다. 그러면 Codex가 뭘 기준으로 고쳐야 하는지 애매해집니다.

codex-harness는 그 대화를 계속 끌고 가지 않기 위한 장치입니다.

확정된 결정, 필요한 문서, 이전 단계의 기록만 남겨서 다음 Codex 세션에 넘깁니다. 말로 이어 붙이지 않고 파일로 넘깁니다.

이 저장소에서는 `scripts/harness/run-phases.py`가 그 일을 합니다. Codex를 호출하고, 확인 명령을 돌리고, 결과 파일을 남깁니다.
...
→ 현재 단계에 필요한 파일만 조합
→ Codex가 새 세션에서 작업
→ 확인 명령과 결과 파일로 완료 여부 확인
```

코드는 여전히 Codex가 고칩니다. 다만 작업의 기준까지 Codex에게 맡기지는 않습니다.

무엇을 만들지, 어디까지 고칠지, 어떤 명령으로 확인할지는 미리 파일에 박아 둡니다. Codex는 그 안에서 작업하고, 완료 여부는 말이 아니라 기록으로 확인합니다.
