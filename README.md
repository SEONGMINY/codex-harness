...
Codex와 긴 작업을 하다 보면 처음에 정한 기준을 계속 지키기 어렵습니다.

처음에 정한 기준, 중간에 버린 선택지, 나중에 바뀐 결정이 한 대화 안에 같이 남습니다. 작업이 길어질수록 Codex가 무엇을 기준으로 고쳐야 하는지 흐려집니다.

codex-harness는 이 문제에서 시작했습니다.

긴 대화를 계속 이어 붙이는 대신, 작업 기준을 파일로 남깁니다. 요구사항을 정리하고, 만들지 말아야 할 범위를 걸러내고, phase별 작업 지시서와 확인 명령을 만듭니다. 그 다음 `scripts/harness/run-phases.py`가 필요한 파일만 모아 새 Codex 세션을 실행합니다.

codex-harness를 쓰면 이렇게 작업합니다.
